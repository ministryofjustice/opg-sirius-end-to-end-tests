...
```shell
docker compose run --rm cypress test -- --spec "cypress/e2e/user-management/navigation.cy.js"
```

you can also run whole folders/ subfolders this way, e.g. 
```shell
docker compose run --rm cypress test -- --spec "cypress/e2e/supervision/clients/*"
```
