...
test.describe("Log in page", () => {
  ...
  test(
    "should fail to validate an empty username and password",
    { tag: "@common @login" },
    async ({ page }) => {
      await page.goto("/old-login", { waitUntil: "networkidle" });
      ...
      await expect(
        page.locator("text=Value is required and can't be empty"),
      ).toBeVisible();
  ...
  test(
    "should not log in with an invalid username and password",
    { tag: "@common @login" },
    async ({ page }) => {
      await page.goto("/old-login", { waitUntil: "networkidle" });
  ...
  test(
    "should redirect to the LPA dashboard when an LPA user successfully logs in",
    { tag: "@common @login" },
    async ({ page }) => {
  ...
  test(
    "should redirect to the Supervision dashboard when an Supervision user successfully logs in",
    { tag: "@common @login" },
    async ({ page }) => {
      await page.goto("/oauth/login");
