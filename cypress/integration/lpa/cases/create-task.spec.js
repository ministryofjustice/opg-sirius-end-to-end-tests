describe('Create a task', { tags: ["@lpa", "@smoke-journey"] }, () => {
  ...
  it('should show the task', () => {
    ...
    cy.intercept({ method: 'GET', url: '/api/v1/persons/*/tasks?*' }).as('tasksRequest');

    cy.wait(['@personRequest', '@tasksRequest']);
    ...
    cy.wait('@tasksRequest').its('response.statusCode').should('equal', 200);
