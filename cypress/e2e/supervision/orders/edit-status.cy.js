    ...
    it("successfully edits the order status for a supervised order and creates a timeline event", () => {
      ...
      cy.get("@order").then(({ id: orderId }) => {
        cy.get("@client").then(({ id: clientId }) => {
          ...
          cy.get('[name="fIELDLABELSORDERSTATUS"][data-core-value=CLOSED]')
            .check({ force: true })
            .should("be.checked");

          cy.get("select[name='fIELDLABELSORDERCLOSUREREASON']").select(
            "Client deceased"
          );
          ...
          cy.get("timeline-order-status-changed .event-order-status")
            .children()
          ...
          cy.get(".event-order-closure-reason").should(
            "contain",
            "CLIENT DECEASED"
          );
        });
    ...
    it("successfully edits the order status for a non-supervised order and creates a timeline event", () => {
      ...
      cy.get("@order").then(({ id: orderId }) => {
        cy.get("@client").then(({ id: clientId }) => {
          ...
          cy.get(
            "#fIELDLABELSORDERSTATUS fieldset > :nth-child(3) > label"
          ).click();
