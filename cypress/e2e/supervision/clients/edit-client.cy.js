...
const editClient = (isCourtReferenceChanged) => {
  return cy.get("@clientId").then((clientId) => {
    ...
    cy.get("@edit-panel").find('input[name="firstName"]').clear().type(firstName);
    cy.get("@edit-panel").find('input[name="lastName"]').clear().type(lastName);
    cy.get("@edit-panel").find('input[name="memorablePhrase"]').clear().type(memorablePhrase);
    if (isCourtReferenceChanged) {
      cy.get("@edit-panel").find('input[name="courtReference"]').clear().type("00000000");
    }
